...
@client.slash_command(
    name="twitch",
    description="Send twitch live status in a channel.",
    default_member_permissions=Permissions(administrator=True),
    dm_permission=False,
)
async def twitch_notification_set(
    interaction: Interaction,
    link: str = SlashOption(
        required=True, description="twitch channel"
    ),
    channel_id: str = SlashOption(
        required=False,
        description="Target Discord channel id to notif twitch live.",
    ),
    custom_message: str = SlashOption(
        required=False,
        description="Use '\\n' for new line. Leave empty to use the default message.",
    ...
    try:
        _logger.info(
            "commands/management: Command 'twitch' was called by "
            + f"'{interaction.user.name}' ({interaction.user.id}) "
            + f"in '{interaction.guild.name}' ({interaction.guild_id}) "
            + f"args: link:{link} channel_id:{channel_id}"
        )
        ...
        result = data.add_twitch_notif_rule(
            interaction.guild_id,
            video.channel_id,
            video.author,
            channel_id,
